...
class Sistema {
    ...
    public void menuPrincipal() {
        ...
        while (true) {
            ...
            switch (opcao) {
                case 1 -> realizarLogin(scanner);
                case 2 -> solicitarCadastroCliente(scanner);
                case 3 -> {
    ...
    private void fazerPedido(Cliente cliente, Scanner scanner) {
        ...
        double preco = calcularPreco(peso, distancia, tempo);
        if (preco == -1) {
            System.out.println("Erro ao calcular o preço.");
            return;
        }

        Pedido pedido = new Pedido(cliente, peso, cepOrigem, cepDestino, distancia, tempo, preco);
        pedidos.add(pedido);
    ...
    private void cadastrarEntregador(Scanner scanner) {
        ...
        // Cria um novo entregador e adiciona à lista de usuários
    ...
    private void atualizarDadosEntregador(Entregador entregador, Scanner scanner) {
        ...
        System.out.print("Placa da moto atual: " + entregador.getPlacaMoto() + ". Nova placa (ou pressione Enter para manter): ");
        String placaMoto = scanner.nextLine();
        if (!placaMoto.isEmpty()) {
            entregador.setPlacaMoto(placaMoto);
        }
